...
#define DEFAULT_SLEEP_TIME_MS 100 // Default sleep time in milliseconds
...
OPERATE_RET at_client_init(char *transport_name)
{
    ...
    // tal_system_sleep(1000); // Allow some time for the thread to start
    while (sg_at_client.status != AT_CLIENT_STATUS_READY) {
        tal_system_sleep(100); // Wait for the thread to initialize
    }
